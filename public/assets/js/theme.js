(function ($) {
  ...
  function gallery_isotope() {
    if ($(".blog_area").length) {
      ...
      $(".g_fillter ul li").on("click", function () {
        ...
        $(".blog_item_inner").isotope({
          filter: selector,
          animationOptions: { duration: 450, easing: "linear", queue: false },
        });
        return false;
      });
    }
  ...
  if ($(".apps_craft_animation").length > 0) {
    $(".apps_craft_animation").parallax({ scalarX: 10.0, scalarY: 10.0 });
  }
  ...
  $(window).on("load", function () {
    if ($(".scroll_body").length) {
      $(".scroll_body").mCustomScrollbar({ theme: "dark-3", setHeight: true });
    }
  });
